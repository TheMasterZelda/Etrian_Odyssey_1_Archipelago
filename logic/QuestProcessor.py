...
class QuestProcessor:
    ...
    def __can_reach_region_requirement(self, param: CanReachRegion, logic_data: AllLogicData, state: StateInterface) -> bool:
        return state.can_reach_region(param.region)

    def __can_obtain_material_requirement(self, param: CanObtainMaterial, logic_data: AllLogicData, state: StateInterface) -> bool:
    ...
    def __has_quest_item_requirement(self, param: HasQuestItem, logic_data: AllLogicData, state: StateInterface) -> bool:
        raise Exception("__has_quest_item_requirement")

    def __has_class_of_level_requirement(self, param: HasClassOfLevel, logic_data: AllLogicData, state: StateInterface) -> bool:
    ...
    def __has_class_and_skill_requirement(self, param: HasClassAndSkill, logic_data: AllLogicData, state: StateInterface) -> bool:
        class_data = logic_data.class_data.class_as_dict[param.class_name]
        if not class_data.class_unlocked:
            return False
    ...
    def __can_solo_enemy_requirement(self, param: CanSoloEnemy, logic_data: AllLogicData, state: StateInterface) -> bool:
    ...
    def __can_fill_x_monster_codex_entries_requirement(self, param: CanFillXMonsterCodexEntries, logic_data: AllLogicData, state: StateInterface) -> bool:
    ...
    def __can_fill_x_item_compendium_entries_requirement(self, param: CanFillXItemCompendiumEntries, logic_data: AllLogicData, state: StateInterface) -> bool:
        currently_fillable_count = len(logic_data.compendium_logic_data.fillable_compendium_entries)
        return currently_fillable_count >= param.entries_count

    def __can_defeat_encounter_requirement(self, param: CanDefeatEncounter, logic_data: AllLogicData, state: StateInterface) -> bool:
        return self.encounter_battle_processor.can_defeat_enemy_group(param.enemies, state, logic_data)

    quest_completion_requirement_lookup: dict[QuestCompletionRequirementType, Callable[[QuestProcessor, QuestCompletionRequirement, AllLogicData, StateInterface], bool]] = {
    ...
    def __requirement_met(self, requirement: QuestCompletionRequirement, logic_data: AllLogicData, state: StateInterface) -> bool:
    ...
    def __can_reach_floor(self, floor_number: int, logic_data: AllLogicData,  state: StateInterface) -> bool:
    ...
    def can_start_quest(self, quest_id: int, logic_data: AllLogicData, state: StateInterface) -> bool:
        ...
        if quest_data.quest_unlock_requirement == QuestRequirement.NONE:
            ...
            return state.has_item(key_item_data.name)
    ...
    def can_complete_quest(self, quest_id: int, logic_data: AllLogicData, state: StateInterface) -> bool:
        if self.region_cache is None:
            self.region_cache = set(state.get_regions())
